...
import TuringClaw.channels.matrix as matrix_module
from TuringClaw.bus.events import OutboundMessage
from TuringClaw.bus.queue import MessageBus
from TuringClaw.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from TuringClaw.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("TuringClaw.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "TuringClaw.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("TuringClaw.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "TuringClaw.channels.matrix.asyncio.create_task", _fake_create_task
    )
