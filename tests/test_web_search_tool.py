...
from TuringClaw.agent.tools.web import WebSearchTool
from TuringClaw.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="TuringClaw", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("TuringClaw.agent.tools.web.DDGS", MockDDGS, raising=False)
    import TuringClaw.agent.tools.web as web_mod
