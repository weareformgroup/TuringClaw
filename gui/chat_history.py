...
class ChatHistoryManager:
    ...
    def start_session(self, provider=None, model=None):
        ...
        with self._lock:
            ...
            now = datetime.now()
            # 加微秒防重名 (连续创建时)
            session_id = now.strftime("%Y%m%d_%H%M%S_") + f"{now.microsecond:06d}"
            self._session = {
    ...
    def get_recent_messages(self, n: int = 10) -> list:
        """获取当前会话最近的 N 条消息（用于多轮上下文）

        Returns:
            list of {"role": str, "content": str} dicts
        """
        with self._lock:
            if self._session is None or not self._session.get("messages"):
                return []
            return [
                {"role": m["role"], "content": m["content"]}
                for m in self._session["messages"][-n:]
            ]
